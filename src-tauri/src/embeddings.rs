...
use crate::config::{AppConfig, user_data_dir};
...
fn get_embedder() -> Result<&'static Mutex<TextEmbedding>, String> {
    ...
    // SAFETY: `EMBEDDER.set()` vient de réussir sur un `OnceLock` vide.
    // `OnceLock::get()` retourne donc obligatoirement `Some` après ce `set()`,
    // y compris en appel concurrent (le second `set()` échoue silencieusement
    // et le `get()` suivant retourne la valeur du premier thread gagnant).
...
pub fn get_groq_api_key() -> Result<String, String> {
    ...
    // 2. Lecture depuis config.json (prod)
    ...
    // 3. Lecture depuis .env.local ou .env
    let possible_paths = [
...
pub fn get_groq_model() -> String {
    ...
    // 3. Modèle par défaut supporté par le compte Groq
