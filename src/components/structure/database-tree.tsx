...
async function treeAction(action: string, path: string, source: string, name?: string, repository?: string) {
  ...
  const res = await fetch('/api/tree-actions', {
    method: 'POST',
  ...
  console.log('[DatabaseTree] treeAction result', { action, path, source, ok: res.ok, data });
  return res.ok ? data : { success: false, error: data.error || `Erreur ${res.status}` };
}
...
export function DatabaseTree({ source, onSelect, selectedPath, webAvailable = true, activeRepo }: DatabaseTreeProps) {
  ...
  const isLocalEditable = source === 'local' && activeRepo !== null && activeRepo !== undefined && activeRepo !== '.data' && !activeRepo.endsWith('/.data') && activeRepo.startsWith('repositories/');
  ...
  const loadStructure = useCallback(async (path: string = '', signal?: AbortSignal) => {
    try {
      ...
      if (activeRepo) params.set('repository', activeRepo);

      console.log('[DatabaseTree] loadStructure', { source, path, activeRepo });
      const response = await fetch(`/api/structure?${params.toString()}`, { cache: 'no-store', signal });
      const data = await response.json();
      console.log('[DatabaseTree] loadStructure result', { source, path, success: data.success, sourceUsed: data.sourceUsed, topLevelNodes: data.data?.length, firstNodeChildren: data.data?.[0]?.children?.length });

      if (!data.success) {
        if (data.available === false) {
          setAvailable(false);
          setError(data.error || 'BDD non disponible');
        } else {
          setAvailable(true);
      ...
      setAvailable(true);
      const deduped = dedupeTree(data.data || []);
      if (!path) {
        setNodes(deduped);
      }
      return deduped;
