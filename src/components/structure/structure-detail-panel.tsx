    ...
    const timeoutId = setTimeout(() => {
      const fetchContent = async () => {
        ...
        try {
          ...
          // Only send repository for local source; web source uses .data/ as canonical reference
          if (source === 'local' && repository) params.set('repository', repository);
          const res = await fetch(`/api/file-content?${params.toString()}`);
          const data = await res.json();
          if (cancelled) return;
          if (data.success) {
            setContent({ kind: data.kind, content: data.content, size: data.size, mimeType: data.mimeType });
            if (data.sourceUsed === 'local' && source === 'web') {
              setError('Affichage local de secours (BDD web incomplète)');
            }
          } else {
            setError(data.error || 'Erreur de lecture');
          }
        } catch (err) {
